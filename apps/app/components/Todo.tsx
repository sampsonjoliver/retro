...
class TodoView extends React.Component<Props, State> {
  ...
  public render() {
    ...
    return (
      <>
        <Hammer
          onPress={() => this.setReorderEnabled(true)}
          onPan={e => this.setPan(e)}
          onSwipe={e => console.log(`Got event: ${e.type} ${e.deltaX}`)}
          onPanStart={() => this.setPanEnabled(true)}
          onPanEnd={() => this.resetPan()}
          options={{
            recognizers: {
              pan: {
                direction: this.state.isReorderEnabled
                  ? DIRECTION_VERTICAL
